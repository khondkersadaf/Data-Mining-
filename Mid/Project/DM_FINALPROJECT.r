...
for (i in 1:k) {
  ...
  decision_tree_info_gain <- rpart(formula(paste(target_var, "~", paste(predictor_cols, collapse = "+"))),
                                   data = train_data,
                                   method = "class",
                                   parms = list(split = "information"))
  
  # Fit the decision tree model with Gini Index
  decision_tree_gini <- rpart(formula(paste(target_var, "~", paste(predictor_cols, collapse = "+"))),
                              data = train_data,
                              method = "class",
                              parms = list(split = "gini"))
  
  # Fit the decision tree model with Gain Ratio
  decision_tree_gain_ratio <- rpart(formula(paste(target_var, "~", paste(predictor_cols, collapse = "+"))),
                                    data = train_data,
                                    method = "class",
  ...
  accuracy_info_gain[i] <- mean(predictions_info_gain == test_data$Index)
  accuracy_gini[i] <- mean(predictions_gini == test_data$Index)
  accuracy_gain_ratio[i] <- mean(predictions_gain_ratio == test_data$Index)
  
  # Create confusion matrix for each criterion
  confusion_matrices_info_gain[[i]] <- table(Actual = test_data$Index, Predicted = predictions_info_gain)
  confusion_matrices_gini[[i]] <- table(Actual = test_data$Index, Predicted = predictions_gini)
  confusion_matrices_gain_ratio[[i]] <- table(Actual = test_data$Index, Predicted = predictions_gain_ratio)
}
...
options(repr.plot.width = 10000, repr.plot.height = 5000)  # Adjust width and height as needed
